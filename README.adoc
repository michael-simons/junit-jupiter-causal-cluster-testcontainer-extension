...
<2> Mark a field as `@CausalCluster`. The field can be of type `URI` or `String` and will contain a `bolt+routing` uri into the cluster
...
To retrieve the whole cluster, annotate exactly one field of type `Cluster` with `@CausalCluster`.
